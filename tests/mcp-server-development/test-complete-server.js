...
async function testCompleteServer() {
  ...
  console.log('🚀 Testing AI Framework Complete Server');
  console.log('='.repeat(60));

  const completeConfig = {
    command: 'node',
    args: ['./ai-framework-mcp-server/dist/index-complete.js'],
    cwd: '.',
    env: {}
  };

  console.log('🔍 Testing complete server with all 25+ AI Framework tools...');
  const result = await tester.testServerWithValidation('ai-framework-complete', completeConfig);
  
  if (result) {
    console.log('✅ Complete server test passed!');
    console.log('🎯 All AI Framework tools are ready for Kiro');
    console.log('\n📊 Available Tools:');
    ...
    console.log('\n🎉 Ready to deploy to Kiro!');
  } else {
    console.log('❌ Complete server test failed');
    console.log('🔧 Need to debug the complete implementation');
  }
