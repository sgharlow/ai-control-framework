...
## Cohort A — pre-discipline era

| Repo | SHA | M1 mock hits (files) | M2 unverifiable claims | M3 CI real | M4 builds | M5 DRS | M6 license | M6 lockfile |
|---|---|---|---|---|---|---|---|---|
| AI-matcher-AWS-hackathon | cc9cb0f | 0 (0) | 4 | no (`no-workflows-dir`) | yes — python-compileall rc=0 | 35 | no | n/a |
| AutoSpecAI-Hackathon | ad53e5f | 0 (0) | 0 | no (`failure`) | NO — node: npm install --ignore-scripts failed | 35 | no | no |
| kiro-living-docs-devpost | 74e40cb | 2 (1) | 0 | no (`failure`) | yes — node+tsc projects=1 rc=0 | 30 | yes | yes |
| gemma-health | e0d953b | 0 (0) | 0 | no (`no-workflows-dir`) | yes — bash-n files=2 rc=0 | 23 | yes | n/a |
| trivia60-aws-game-builder | ebbe881 | 0 (0) | 0 | no (`no-workflows-dir`) | yes — node-check files=33 rc=0 | 23 | no | yes |
| gamejam26 | 5969e44 | 0 (0) | 0 | yes (`success`) | n/a — godot: no static compiler available in this environment | 23 | yes | n/a |
| scrum-web-app | 02c4098 | 0 (0) | 0 | no (`no-workflows-dir`) | yes — node+tsc projects=1 rc=0 | 27 | no | yes |
| dragon-haven | 9a29a0f | 6 (2) | 0 | no (`no-workflows-dir`) | yes — python-compileall rc=0 | 20 | no | n/a |
| AICIN | 032ac74 | 0 (0) | 1 | no (`no-workflows-dir`) | NO — node+tsc projects=9 rc=1 | 23 | no | no |
| kiro-rails | aebb642 | 0 (0) | 0 | no (`no-workflows-dir`) | yes — bash-n files=9 rc=0 | 31 | yes | n/a |
| info-worker-demo | d379030 | 0 (0) | 0 | no (`no-workflows-dir`) | n/a — no recognized ecosystem (no package.json/godot/python/sh) | 31 | no | n/a |
| ticket-quality | eaff8e5 | 0 (0) | 0 | yes (`success`) | yes — python-compileall rc=0 | 23 | no | n/a |
| ai-pr-bot | 9d1549f | 13 (2) | 0 | no (`no-workflows-dir`) | NO — node+tsc projects=1 rc=1 | 29 | yes | yes |

## Cohort B — verification-discipline era

| Repo | SHA | M1 mock hits (files) | M2 unverifiable claims | M3 CI real | M4 builds | M5 DRS | M6 license | M6 lockfile |
|---|---|---|---|---|---|---|---|---|
| skillcrossroads | 65723ba | 0 (0) | 0 | yes (`success`) | NO — node+tsc projects=3 rc=1 | 32 | yes | yes |
| relay | 43b7e28 | 8 (5) | 1 | no (`no-workflows-dir`) | yes — node+tsc projects=1 rc=0 | 29 | yes | yes |
| orbis-exchange | e0e88b1 | 0 (0) | 1 | no (`no-workflows-dir`) | NO — node+tsc projects=3 rc=1 | 39 | yes | yes |
| mdlink-check | cdd5a8e | 0 (0) | 0 | yes (`success`) | yes — node-check files=4 rc=0 | 32 | yes | no |
| orchestra-lite | d3a20c6 | 0 (0) | 0 | yes (`success`) | yes — node-check files=1 rc=0 | 28 | yes | no |
| mr-sentinel | 533169b | 0 (0) | 0 | yes (`success`) | yes — python-compileall rc=0 | 40 | yes | n/a |
...
| Repos measured | 13 | 6 |
| Median DRS (M5) | 27 | 32 |
| % buildable (M4, n/a excluded) | 73% (8/11) | 67% (4/6) |
| % with real CI (M3: workflows + latest run success) | 15% (2/13) | 67% (4/6) |
| % with mock residue (M1 hits > 0) | 23% (3/13) | 17% (1/6) |
| % with unverifiable claims (M2 flags > 0) | 15% (2/13) | 33% (2/6) |
| % with LICENSE (M6) | 38% (5/13) | 100% (6/6) |
